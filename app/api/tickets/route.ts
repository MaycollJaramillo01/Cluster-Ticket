...
export async function POST(req: NextRequest) {
  try {
    ...
    let ticket = await prisma.ticket.create({ data: { ...parsed.data, assignedToId: assignee.id, createdById: currentUser.id, tags: JSON.stringify(parsed.data.tags), activityLog: { create: { userId: currentUser.id, action: 'TICKET_CREATED', newValue: 'Nuevo' } } } })
    if (files.length) {
      try {
