...
export function AppShell({children}:{children:React.ReactNode}){
 ...
 const title=nav.find(n=>n.href==='/'?path==='/':path.startsWith(n.href))?.label||'Cluster Flow'
 ...
 return <div className="app"><div className={`drawer-backdrop ${open?'open':''}`} onClick={()=>setOpen(false)}/><aside className={`sidebar ${open?'open':''}`}>
  <div className="brand"><div className="brand-mark"><BrandMark size={20}/></div><div><b>Cluster Flow</b><span>Agency workspace</span></div><button className="btn btn-ghost btn-icon mobile-menu" onClick={()=>setOpen(false)} aria-label="Cerrar menú"><X/></button></div>
